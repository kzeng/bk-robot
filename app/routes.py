    ...
    try:
        if action == 0:  # Photo mode
            for marker in marker_list:
                ...
                # Take photos from all cameras
                photo_result = obs_control.take_screenshot_all_cameras(marker)
                if photo_result['status'] != 'OK':
                    raise Exception(f"Failed to take photos at marker {marker}")
            ...
            move_result = robot_control.send_command(f'/api/move?marker={first_marker}')
            if move_result.get('status') != 'ok':
                raise Exception(f"Failed to move to first marker {first_marker}: {move_result.get('message')}")
            
            # Start recording
            start_result = obs_control.start_recording(marker_list)
            if start_result['status'] != 'OK':
                raise Exception(f"Failed to start recording: {start_result.get('message')}")
            
            # Move through remaining markers if any
            if len(marker_list) > 1:
                other_markers = ",".join(marker_list[1:])
                move_result = robot_control.send_command(f'/api/move?marker={other_markers}')
                if move_result.get('status') != 'ok':
                    raise Exception(f"Failed to move through markers: {move_result.get('message')}")
            
            # Stop recording
            stop_result = obs_control.stop_recording()
            if stop_result['status'] != 'OK':
                raise Exception(f"Failed to stop recording: {stop_result.get('message')}")
            
            if stop_result.get('file_path'):
                file_paths.append(stop_result['file_path'])
        ...
        return jsonify({
            ...
            'message': 'Task completed successfully' if status else error_message,
