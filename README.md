...
BK Robot is a Flask application for library inventory robot control. The current hardware profile uses the robot base plus RTSP IP cameras captured through OpenCV.

## Features

- Robot movement and status control through the WATER robot HTTP API.
- Inventory task creation, execution, and task-log tracking.
- Multi-camera RTSP configuration and OpenCV photo/video capture.
- Marker configuration and task result file management.
...
- `ROBOT_IP`, `ROBOT_PORT`: robot base connection settings.
- `FTP_HOST`, `FTP_PORT`, `FTP_USER`, `FTP_PASS`, `FTP_MOCK_MODE`: upload settings.
